...
fun send_punished_list (gt : game_table) (l : list int) : transaction {} =
    send_public_message gt (GeneralRsp (PlayersPunished l))
...
fun vote_failed (gt : game_table) : transaction {} =
    ...
    in  send_public_message gt (GeneralRsp (NewGovt state));
        (case state of
...
fun winners (gt : game_table) (winners : side) : transaction {} =
    ...
    case game_end_state_o of
        None => return {}
      | Some game_end_state =>
        send_public_message gt (GeneralRsp (GameEndState game_end_state))
